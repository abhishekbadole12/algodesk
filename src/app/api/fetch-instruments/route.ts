...
export async function GET() {
  try {
    ...
    // ✅ 2. Check file cache (today’s date)
    ...
    // 🔄 3. Fetch new data
    const [typeAResponse, typeBResponse] = await Promise.all([
      axios.get(MSTOCK_SCRIPT_MASTER_TYPE_A),
      axios.get(MSTOCK_SCRIPT_MASTER_TYPE_B, {
        headers: {
    ...
    // Parse both datasets
    const typeAData = Papa.parse(typeAResponse.data, { header: true }).data;
    const typeBData = typeBResponse.data;

    // Normalize and merge
    const combined: Instrument[] = [
      ...typeAData.map(normalizeTypeA),
